...
import (
	...
	. "zombiezen.com/go/bytesreplacer"
)
...
func TestReplacer(t *testing.T) {
	...
	for i, tc := range testCases {
		// TODO(light): test under different capacity conditions
		...
		got := buf.String()
		if got != tc.out {
			t.Errorf("%d. Write(%q) wrote %q, want %q", i, tc.in, got, tc.out)
			continue
		}
		if n != len(tc.out) {
			t.Errorf("%d. Write(%q) wrote correct string but reported %d bytes; want %d (%q)",
...
func BenchmarkGenericMatch2(b *testing.B) {
	str := Repeat([]byte("It&apos;s &lt;b&gt;HTML&lt;/b&gt;!"), 100)
	for i := 0; i < b.N; i++ {
...
func BenchmarkByteStringReplacerWriteString(b *testing.B) {
	...
	buf := new(Buffer)
	for i := 0; i < b.N; i++ {
		htmlEscaper.Write(buf, []byte(str))
		buf.Reset()
	}
}
...
func BenchmarkByteReplacerWriteString(b *testing.B) {
	...
	buf := new(Buffer)
	for i := 0; i < b.N; i++ {
		capitalLetters.Write(buf, []byte(str))
		buf.Reset()
	}
}
...
func BenchmarkByteByteReplaces(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		Replace(Replace([]byte(str), []byte{'a'}, []byte{'A'}, -1), []byte{'b'}, []byte{'B'}, -1)
	}
}
